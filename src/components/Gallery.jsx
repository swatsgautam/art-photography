...
const Gallery = () => {
  ...
  const handleFavoriteToggle = (image) => {
    ...
    if (isFavorite) {
      // Remove from favorites if it's already liked
      toggleFavorite(image, false); // Pass false to indicate removal from favorites
    } else {
      // Add to favorites if it's not liked yet
      toggleFavorite(image, true); // Pass true to indicate addition to favorites
    }
  };
          ...
          filteredImages.map(image => (
            <div className="grid-item" key={image.id} onClick={() => handleImageClick(image)}> {/* Trigger popup on click */}
              ...
              <span
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  handleFavoriteToggle(image); // Toggle like/unlike
                }}
              >
                ♥
              </span>
